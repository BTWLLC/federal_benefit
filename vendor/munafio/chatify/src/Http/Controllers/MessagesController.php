...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($fetch) {
            // Fetch the user avatar using Chatify
            $userAvatar = Chatify::getUserWithAvatar($fetch)->avatar;
    
            // Determine the profile image URL
            $profileImageUrl = $fetch->profile->image ? asset('upload/profile/' . $fetch->profile->image) : 'images/profile/default-profile-image.jpg';
        }
        return Response::json([
            'favorite' => $favorite,
            'fetch' => $fetch ?? null,
            'user_avatar' => $profileImageUrl ?? $userAvatar ?? null,
        ]);
    ...
    public function getContacts(Request $request)
    {
        ...
        ->select('users.id', 'users.first_name', 'users.last_name', 'users.email', 'users.created_at', 'users.updated_at', DB::raw('MAX(ch_messages.created_at) as max_created_at'))
        ->orderBy('max_created_at', 'desc')
        ->groupBy('users.id', 'users.first_name', 'users.last_name', 'users.email', 'users.created_at', 'users.updated_at')
